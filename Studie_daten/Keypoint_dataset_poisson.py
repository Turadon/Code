...
class Keypoint_dataset_poisson(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            ...
            matched_poisson = torch.poisson(matched*255)
            keypoints = transformed['keypoints']
            keypoints = torch.from_numpy(np.float32(keypoints).flatten())

        return matched_poisson.type(torch.float)/255, keypoints.unsqueeze(0)
...
def get_loader_keypoint_simulated(BATCH_SIZE):
